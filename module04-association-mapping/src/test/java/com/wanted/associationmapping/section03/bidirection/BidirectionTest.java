...
public class BidirectionTest {
    ...
    *  하지만 객체는 서로 다른 두 단방형 참조를 합쳐야 양방향 조회가 가능하다
    *  따라서 두 개의 연관 관계 중 주인을 설정해야 하며
    *  주인을 설정하는 속성은 mapped by 로 설정하고 주인 설정 기준은 외래키를 가지고 있는 엔터티가 주인이 되는 것이다 ㅍ
    *  따라서 Menu와 Category 엔터티는 외래키를 가지고 있는 Menu 엔터티가 주인이다   */
