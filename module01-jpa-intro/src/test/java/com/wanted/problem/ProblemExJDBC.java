...
public class ProblemExJDBC {
    ...
    /*comment
    *  테스트 코드 기반으로 JPA를 사용하지 않을 때 발생할 수 있는 문제를 예시로 알아보자 */

    /*comment
    *  Test 전용 클래스란 ?
    *  @Test 라는 어노테이션을 1개 이상 가지고 있는 클래스를 의미한다
    *  테스트 메서드는 반환값이 없으며 void로 작성한다
    *  메서드별로 독립적인 실행이 가능하며 우리가 만든 기능을 테스트 하는 것에 초점을 맞춘다
    *  또한 접근제어자는 사용하지 않아도 되며
    *  단 private는 허용되지 않는다 */

    // DB 연결 시 필요한 Con 객체 생성
    ...
    *  2. SQL에 의존적인 개발
    *  - ERD에 맞춘 개발 방식
    *  - OOP(객체지향프로그래밍) 이 아닌 SQL 의존 개발이 된다
    *   */
...
//        }
    ...
    *  */
    ...
    void equalsTwoObject() throws SQLException {
        ...
        /*comment
        *  중요 ★
        *  하나의 DB에서 동일한 행에 해당하는 데이터를 꺼냈는데
